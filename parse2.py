    ...
    def print_parse_trees(self) -> None:
        ...
        def build_tree(item: Item) -> str:
            ...
            while cur.dot_position > 0:
                ...
                etype, payload = cur.edge

                if etype == "scan":
                    prev = payload  # Item before scanning the terminal
                    term = prev.rule.rhs[prev.dot_position]
                    children.append(term)
                    cur = prev  # move left
                elif etype == "attach":
                    child_complete: Item = payload  # completed child constituent
                    children.append(build_tree(child_complete))
                    cur = cur.edge_attach
                    if cur is None:
    ...
    def _attach(self, item: Item, position: int) -> None:
        ...
        for customer in self.cols[mid].all():  # could you eliminate this inefficient linear search?
            if customer.next_symbol() == item.rule.lhs:
                ...
                new_item.setEdge(("attach", item))
                new_item.setEdgeAttach(customer)
                new_weight = new_item.weight + item.weight
                new_item.setWeight(new_weight)
                self.cols[position].push(new_item)
...
class Item:
    ...
    dot_position: int
    start_position: int
    edge: Optional[Tuple[str, Item]] = None
    edge_attach: Optional[Item] = None
    weight: float = 0.0
    ...
    def setEdge(self, edge: Tuple[str, Item]) -> None:
        """Set the edge information for this item."""
        object.__setattr__(self, 'edge', edge)

    def setEdgeAttach(self, edge_attach: Item) -> None:
        """Set the edge_attach information for this item."""
        object.__setattr__(self, 'edge_attach', edge_attach)

    def setWeight(self, weight: float) -> None:
        """Set the weight information for this item."""
    ...
    def with_dot_advanced(self) -> Item:
        ...
        if new_item.next_symbol() is None:
            object.__setattr__(new_item, 'weight', self.weight + self.rule.weight)
        return new_item
