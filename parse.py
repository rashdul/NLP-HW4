...
class EarleyChart:
    ...
    def printParse(self) -> str:
        """Print the parse with the highest probability, if any. """
        start = {}
        # for debugging
        # for item in self.cols[-1].all():    # the last column
        #     print(f"Column 2 item weight: {item.weight} : {item}")
        for item in self.cols[-1].all():    # the last column
            if (item.rule.lhs == self.grammar.start_symbol   # a ROOT item in this column
                and item.next_symbol() is None               # that is complete
                and item.start_position == 0):               # and started back at position 0
        ...
        def getTree(item: Item) -> str:
            ...
            cursor = item
            # Reverse the edges to get them in RHS order
            while cursor is not None and cursor.edge is not None:
                edges.append(cursor.edge)
                cursor = cursor.prev
            edges.reverse()  # now in the same order as the rule's RHS

            children_strs: List[str] = []
            rhs = item.rule.rhs

            # Walk RHS in lockstep with edges
            # ri = 0
            for kind, payload in edges:
                # sym = rhs[ri]
                if kind == "scan":
                    token = payload  

                    children_strs.append(f"{token}")
                else:  # "attach"
                    child_item = payload  # type: Item
                    children_strs.append(getTree(child_item))
                # ri += 1

            # If the rule is preterminal like X -> token
            # Otherwise combine children
            if len(rhs) == 1 and not self.grammar.is_nonterminal(rhs[0]):
                return f"({item.rule.lhs} {children_strs[0]})"
            else:
    ...
    def _scan(self, item: Item, position: int) -> None:
        ...
        if position < len(self.tokens) and self.tokens[position] == item.next_symbol():
            new_item = item.with_dot_advanced(prev=item, edge=("scan", self.tokens[position]), weight=item.weight)
...
class Item:
    ...
    edge: Optional[Tuple[str, object]] = None
    weight: float = 0.0
    ...
    def with_dot_advanced(self, prev = None, edge = None, weight = None) -> Item:
        if self.next_symbol() is None:
            raise IndexError("Can't advance the dot past the end of the rule")
        if weight is None:
            weight = self.weight
        return Item(rule=self.rule, dot_position=self.dot_position + 1, start_position=self.start_position, prev=prev, edge=edge, weight=weight)
